<template>
  <PageHeader :title="$t('样本列表')" :description="$t('浏览集合中的身份样本和人脸向量处理状态。')">
    <el-button type="primary" @click="createSample"><Plus :size="16" />{{ $t('新建样本') }}</el-button>
  </PageHeader>
  ...
  <form class="query-bar sample-query" @submit.prevent="load">
    ...
    <div class="query-field"><label for="limit">{{ $t('每页数量') }}</label><el-input-number id="limit" v-model="query.limit" :min="1" :max="100" /></div>
    <div class="query-field"><label for="order">{{ $t('排序') }}</label><el-select id="order" v-model="query.order"><el-option :label="$t('最早创建')" value="asc" /><el-option :label="$t('最近创建')" value="desc" /></el-select></div>
  ...
  <section class="workspace-panel">
    <div class="panel-heading">
      ...
      <div class="inline-actions">
        <el-button :disabled="query.offset === 0" @click="previous"><ChevronLeft :size="16" />{{ $t('上一页') }}</el-button>
        <span class="offset-label">{{ $t('偏移') }} {{ query.offset }}</span>
        <el-button :disabled="samples.length < query.limit" @click="next">{{ $t('下一页') }}<ChevronRight :size="16" /></el-button>
      </div>
    </div>
    <el-table v-loading="loading" :data="samples" :empty-text="$t('暂无样本')" stripe row-key="sampleId">
        ...
        <template #default="{ row }">
          <div class="face-table">
            <el-table :data="row.faces || []" :empty-text="$t('该样本尚未录入人脸')" size="small">
              ...
              <el-table-column :label="$t('人脸数据')" min-width="180"><template #default="scope">{{ summarize(scope.row.faceData) }}</template></el-table-column>
            </el-table>
          </div>
        </template>
      </el-table-column>
      <el-table-column prop="sampleId" :label="$t('样本 ID')" min-width="170"><template #default="{ row }"><span class="mono">{{ row.sampleId }}</span></template></el-table-column>
      <el-table-column :label="$t('扩展数据')" min-width="260" show-overflow-tooltip><template #default="{ row }">{{ summarize(row.sampleData) }}</template></el-table-column>
      <el-table-column :label="$t('人脸数')" width="90" align="center"><template #default="{ row }">{{ row.faces?.length || 0 }}</template></el-table-column>
...
function summarize(pairs) {
  if (!pairs?.length) return translate('无')
