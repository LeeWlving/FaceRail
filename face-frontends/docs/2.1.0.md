...
以下接口由 FaceRail 增加，不改变前述 Face Search 2.1 兼容接口。浏览器使用
SCRFD 与 ArcFace 生成 512 维归一化向量后，可只提交向量而不提交查询原图。

## 1、下载模型

| 模型 | 请求方式 | 接口地址 |
| --- | --- | --- |
| SCRFD | GET | `/api/models/scrfd` |
| ArcFace | GET | `/api/models/arcface` |

## 2、使用设备端向量搜索

**接口地址**：`/api/visual/search/embedding`
...
`embedding` 必须包含 512 个数值。响应结构与 `/api/visual/search/do` 完全相同。

## 3、使用设备端向量录入

**接口地址**：`/api/visual/face/create_embedding`
...
`embedding` 必须包含 512 个数值。当集合启用 `storageFaceInfo` 时，
`faceImageBase64` 必须是浏览器对齐后的 112×112 人脸图片；未启用时无需提交。
设备端录入不会上传原始图片，成功响应中的 `embeddingStatus` 为 `ready`。
